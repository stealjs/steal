			...
			if(script.type){
                ...
                for(i=0;i<types.length;i++){
                    if($env.scriptTypes[types[i]]){
                        ...
                        if(script.src){
                            //anonymous type with src means script should not be loaded
                            //defer processing to onLoadUnknownTypeScript callback
                            $env.onLoadUnknownTypeScript(script);
                        }
                    }
                }
            }else{
...
XMLP.prototype._parseAttribute = function(iB, iE) {
    ...
    if( this._needToReplaceEntities(strV) ) {
...
XMLP.prototype._needToReplaceEntities = function(strV){
    var entities = $w.$entityDefinitions;
    for(var entity in entities){
        if( strV.match( "&"+entity+";" ) ) return true;
    }
    return false;
}
